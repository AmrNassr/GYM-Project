...
String amount = advanceamount.getText();
  String name = cmname.getSelectedItem().toString();
    ...
    {
            try{
                String sql = "SELECT MIN(remaining) as remaining FROM gym.advanceamount where serial='"+serial+"' AND date between '"+curr+"' AND '"+end+"'";
             ...
             if(rs.next()){
                 ...
                 int remaining = remain - x;
                  String sql111 = "INSERT INTO gym.advanceamount (name,serial,amount,remaining,date) VALUES "
             ...
             {
                 String sqll = "SELECT salary FROM gym.employee where serial='"+serial+"'";
             pst=(PreparedStatement) con.prepareStatement(sqll);
             rs=pst.executeQuery();
             if(rs.next()){
                 int remianing = Integer.parseInt(rs.getString("salary"));
                 int remain = remianing-x;
                  String sql1 = "INSERT INTO gym.advanceamount (name,serial,amount,remaining,date) VALUES "
   ...
   public void tablelord()
    {
        try {
            String sqll = "SELECT serial,name,amount,remaining,date FROM gym.advanceamount  ORDER BY serial ASC,\n" +
"    remaining DESC;";
              pst=(PreparedStatement) con.prepareStatement(sqll);
            rs=pst.executeQuery();
 ...
 serialid.setText("");
   ...
   public void comboxfromcombo()
      {
            ...
            public void itemStateChanged(ItemEvent event) {
                ...
                Object item = event.getItem();
                     try {
             String sql="SELECT name  FROM gym.employee where serial='"+cmserial.getSelectedItem().toString()+"'";
             pst=(PreparedStatement) con.prepareStatement(sql);
             rs=pst.executeQuery();
             if(rs.next()){
                ...
                if (event.getStateChange() == ItemEvent.DESELECTED) {
                 ...
                 try {
             String sql="SELECT MIN(remaining)as remaining  FROM gym.advanceamount where serial='"+cmserial.getSelectedItem().toString()+"' and date between '"+curr+"' AND '"+end+"'";
               pst=(PreparedStatement) con.prepareStatement(sql);
               rs=pst.executeQuery();
               if(rs.next()){
                   if(rs.getString("remaining") != null && rs.getString("remaining").trim().length() > 0){
                       amount.setText(rs.getString("remaining"));
                        ...
                        String sql9="SELECT salary  FROM gym.employee where serial='"+cmserial.getSelectedItem().toString()+"'";
   ...
   {
        try {
             String sql="SELECT serial  FROM gym.employee order by CONVERT(replace(serial,'EM',''),UNSIGNED INTEGER) ASC ";
             pst=(PreparedStatement) con.prepareStatement(sql);
             rs=pst.executeQuery();
             while(rs.next()){
   ...
   {
        try {
             String sql="SELECT name  FROM gym.employee ";
             pst=(PreparedStatement) con.prepareStatement(sql);
             rs=pst.executeQuery();
             while(rs.next()){
