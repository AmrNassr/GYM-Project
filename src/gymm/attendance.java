...
headerLabel.setHorizontalAlignment(JLabel.CENTER);
         ...
         || dayy == 5 || dayy == 6 || dayy == 7 || dayy == 8 || dayy == 9 )
 {
    ...
    private void initComponents() {
        ...
        sport.setFont(new java.awt.Font("Arial", 1, 18)); // NOI18N
        jPanel10.add(sport, new org.netbeans.lib.awtextra.AbsoluteConstraints(150, 110, 210, 30));

        jLabel21.setBackground(new java.awt.Color(255, 255, 255));
        jLabel21.setFont(new java.awt.Font("Arial", 1, 18)); // NOI18N
        jLabel21.setText("Sport");
        jPanel10.add(jLabel21, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 120, 110, -1));
        ...
        jPanel10.add(save3, new org.netbeans.lib.awtextra.AbsoluteConstraints(320, 180, 120, 50));
        ...
        jPanel10.add(jLabel20, new org.netbeans.lib.awtextra.AbsoluteConstraints(390, 120, 110, -1));

        enddate.setEditable(false);
        enddate.setFont(new java.awt.Font("Arial", 1, 18)); // NOI18N
        jPanel10.add(enddate, new org.netbeans.lib.awtextra.AbsoluteConstraints(490, 120, 210, 30));
    ...
    private void serialKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_serialKeyPressed
      ...
      TimeZone tz = TimeZone.getTimeZone("GMT+02");
        ...
        if(evt.getKeyCode() == KeyEvent.VK_ENTER) {
           try {
                 String sqls="SELECT serial FROM gym.attendance where serial='"+serial.getText()+"' AND date='"+datee+"'";
             pst=(PreparedStatement) con.prepareStatement(sqls);
             rs=pst.executeQuery();
             if(rs.next()){
                 JOptionPane.showMessageDialog(this, "Customer has already been registered");
             }
             else
             {
             ...
             if(rs.next()){
                 ...
                 enddate.setText(rs.getString("too"));
                 sport.setText(rs.getString("sport"));
                 
             }
             else
             {
             JOptionPane.showMessageDialog(this, "Customer didn't pay yet");
             }
              String namee = name.getText();
      String phonee =phone.getText();
      String sportt=sport.getText();
              String sqll2="SELECT * FROM gym.pay where serial='"+serial.getText()+"' order by id DESC";
             pst=(PreparedStatement) con.prepareStatement(sqll2);
             rs=pst.executeQuery();
             if(rs.next()){
              ...
              SimpleDateFormat sdf = new SimpleDateFormat("yyyy-M-dd");
                try {
                  Date  min = sdf.parse(frommm);
                  Date max = sdf.parse(tooo);
                  Date between = sdf.parse(datee);    
                  if(between.compareTo(min) >= 0 && between.compareTo(max) <= 0)
                  {
                      String format = "yyyy-M-dd";
                      long diff = max.getTime() - between.getTime();
 
			int diffDays = (int) (diff / (24 * 60 * 60 * 1000));
                      if(diffDays < 3)
                      {
                           String sqlz = "INSERT INTO gym.attendance (name,serial,phone,sport,date,fulldate) VALUES"
                        + " ('"+namee+"','"+seriall+"','"+phonee+"','"+sportt+"','"+datee+"','"+fulldatee+"')";
                        pst = (PreparedStatement)con.prepareStatement(sqlz );
                        pst.execute();
                           JOptionPane.showMessageDialog(this, "The customer has to pay After : "+diffDays+" Days");
                           clear();
                           tablelord();
                      }
                      else
                      {
                        ...
                        pst = (PreparedStatement)con.prepareStatement(sqlz );
                        pst.execute();
                      JOptionPane.showMessageDialog(this, "Customer Registed Successfully"+ " And Still has : "+diffDays+" Days");
                      clear();
                      tablelord();
                      }
                  ...
                  else
                  {
                      String count=null;
                      int number=0;
                      String sqk = "SELECT mark FROM gym.people where serial = '"+seriall+"'";
             pst=(PreparedStatement) con.prepareStatement(sqk);
             rs=pst.executeQuery();
             if(rs.next()){
                 count=rs.getString("mark");
                 number = Integer.parseInt(count);
                 if(number <2)
                 {
                     int dialogButton = JOptionPane.YES_NO_OPTION;
                       int dialogResult = JOptionPane.showConfirmDialog(this, "customer should pay do you need make exception ?", "Confirmation", dialogButton);
                if(dialogResult == 0) { 
                    try {
             ...
             pst.execute();
             number=number+1;
             String sqla = "UPDATE gym.people SET mark="+number+" where  serial='"+seriall+"'";
             ...
             Logger.getLogger(attendance.class.getName()).log(Level.SEVERE, null, ex);
                 ...
                 {
                  JOptionPane.showMessageDialog(this, "Customer enter gym 2 time without pay");  
                  clear();
                 }
...
private void salary() {
       try {
             String sql="SELECT serial FROM gym.advanceamount group by serial";
             pst=(PreparedStatement) con.prepareStatement(sql);
            rs=pst.executeQuery();
             while(rs.next()){
                String serial = rs.getString("serial");
                  try {
                     String sqllll = "UPDATE gym.advanceamount SET remaining=(select employee.salary from employee where employee.serial=advanceamount.serial) WHERE advanceamount.serial='"+serial+"'";
